...
void Test_Compaction_Stats_Consistency() {
  ...
  assert(report.total_output_files >= 0);

  // After compacting 10 versions of 500 keys,
  // output should be significantly smaller than input
  if (report.bytes_read > 0) {
    double ratio = double(report.bytes_written) / double(report.bytes_read);
    std::cout << "  Write amplification ratio: " << ratio << "\n";
    // Output should be less than input (we eliminated 9/10 versions)
    assert(report.bytes_written <= report.bytes_read);
    Pass("bytes_written <= bytes_read (old versions eliminated)");
  }
